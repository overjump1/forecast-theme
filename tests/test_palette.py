...
def test_both_palettes_define_every_token():
    ...
    it."""
    for name in forecast_theme.token_names():
        assert getattr(forecast_theme.LIGHT, name) is not None, f"LIGHT.{name} is None"
        assert getattr(forecast_theme.DARK, name) is not None, f"DARK.{name} is None"
...
def test_palette_is_hashable():
    ...
    assert hash(forecast_theme.LIGHT) != hash(forecast_theme.DARK)
    assert len({forecast_theme.LIGHT, forecast_theme.DARK, forecast_theme.LIGHT}) == 2
...
def test_ink_renders_both_forms():
    ink = forecast_theme.Ink(120, 92, 66, 0.18)
...
def test_ink_defaults_to_opaque():
    assert str(forecast_theme.Ink(1, 2, 3)) == "rgba(1, 2, 3, 1.0)"


@pytest.mark.parametrize("mode, prefers_light, expected", [
    (forecast_theme.MODE_LIGHT, None, "light"),
    (forecast_theme.MODE_LIGHT, False, "light"),
    (forecast_theme.MODE_DARK, True, "dark"),
    (forecast_theme.MODE_DARK, None, "dark"),
    (forecast_theme.MODE_SYSTEM, True, "light"),
    (forecast_theme.MODE_SYSTEM, False, "dark"),
    (forecast_theme.MODE_SYSTEM, None, "dark"),
    ("garbage", None, "dark"),
])
def test_resolve(mode, prefers_light, expected):
    ...
    assert forecast_theme.resolve(mode, prefers_light=prefers_light).name == expected


def test_use_sets_the_active_palette():
    before = forecast_theme.active()
    try:
        assert forecast_theme.use(forecast_theme.LIGHT) is forecast_theme.LIGHT
        assert forecast_theme.active() is forecast_theme.LIGHT
        assert forecast_theme.use(forecast_theme.DARK) is forecast_theme.DARK
        assert forecast_theme.active() is forecast_theme.DARK
    finally:
        forecast_theme.use(before)


def test_modes_are_the_persisted_vocabulary():
    """One settings vocabulary across every app in the family."""
    assert forecast_theme.MODES == ("light", "dark", "system")
    assert forecast_theme.PALETTES == {"light": forecast_theme.LIGHT, "dark": forecast_theme.DARK}


def test_geometry_is_not_per_mode():
    """A radius is not an appearance, so it is a module constant rather than a palette field."""
    assert forecast_theme.RADIUS_PILL_LG == 20
    assert (forecast_theme.S1, forecast_theme.S7) == (4, 48)
    assert not any(n.startswith("radius") for n in forecast_theme.token_names())
