...
import paperskin
from paperskin import sheet, testing


@BOTH
def test_qss_compiles(pal):
    for glass in (True, False):
        text = paperskin.base_qss(pal, glass=glass)
...
def test_glass_flag_changes_the_window_fill(pal):
    ...
    assert "QMainWindow { background: transparent; }" in paperskin.base_qss(pal, glass=True)
    assert f"QMainWindow {{ background: {pal.void}; }}" in paperskin.base_qss(pal, glass=False)
...
def test_tone_rules_accepts_app_hosts(pal):
    ...
    extra = paperskin.tone_rules(pal, hosts=("#CardName", "#RowTitle"))
    for tone in paperskin.Tone:
...
def test_contract_names_are_all_styled(pal):
    ...
    text = paperskin.base_qss(pal)
    missing = [n for n in paperskin.CONTRACT_NAMES if f"#{n}" not in text]
...
def test_font_faces_reach_the_sheet(pal):
    text = paperskin.base_qss(pal, "TestSans", "TestDisplay", "TestMono")
...
def test_no_pixel_font_sizes(pal):
    ...
    assert not re.findall(r"font-size:\s*[\d.]+px", paperskin.base_qss(pal))
